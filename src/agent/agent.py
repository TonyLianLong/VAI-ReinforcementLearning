...
import utils
from agent.encoder import make_encoder, all_encoders, adapter_load_state_dict

LOG_FREQ = 10000


def make_agent(obs_shape, action_shape, args):
    print("Update rewards:", args.update_rewards)
    return SacSSAgent(
        obs_shape=obs_shape,
        ...
        ss_lr=args.ss_lr,
        update_rewards=args.update_rewards,
        ...
        adapt_aug=args.adapt_aug,
        adapt_observation=args.adapt_observation
    )
...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        encoder_feature_dim, log_std_min, log_std_max, num_layers, num_filters, num_shared_layers, fine_tune, adapt_aug, adapt_observation
    ):
        super().__init__()

        self.encoder = make_encoder(
            obs_shape, encoder_feature_dim, num_layers,
            num_filters, num_shared_layers, adapt_aug=adapt_aug, adapt_observation=adapt_observation
        )
        ...
        if self.fine_tune:
            # freeze the actor except the last layer
            for trunk in self.trunk[:-1]:
...
class RewardFunction(nn.Module):
    """MLP for fitted rewards model."""
    def __init__(self, obs_dim, output_dim, hidden_dim):
        super().__init__()
...
class Critic(nn.Module):
    ...
    def __init__(
        self, obs_shape, action_shape, hidden_dim,
        encoder_feature_dim, num_layers, num_filters, num_shared_layers, adapt_aug, adapt_observation
    ):
        super().__init__()
...
class SacSSAgent(object):
    ...
    def __init__(
        ...
        self.actor = Actor(
            ...
            num_layers, num_filters, num_layers, fine_tune, adapt_aug=adapt_aug, adapt_observation=adapt_observation
        ).cuda()

        self.critic = Critic(
            obs_shape, action_shape, hidden_dim,
            encoder_feature_dim, num_layers, num_filters, num_layers, adapt_aug=adapt_aug, adapt_observation=adapt_observation
        ).cuda()
        ...
        if use_rot or use_inv or self.use_rewards:
            ...
            # fitting reward
            # NOTE: may share layers with inverse dynamics
            self.fitted_reward = RewardFunction(encoder_feature_dim, 1, hidden_dim).cuda()
            self.fitted_reward.apply(weight_init)
        ...
        # ss optimizers
        self.init_ss_optimizers(encoder_lr, ss_lr, reward_lr)
        ...
        if adapt_observation:
            adapter_load_state_dict(torch.load(adapter_checkpoint))

        if fine_tune and self.ss_encoder:
            for param in self.ss_encoder.parameters():
                param.requires_grad_(False)

    def init_ss_optimizers(self, encoder_lr=1e-3, ss_lr=1e-3, reward_lr=1e-3):
        ...
        if self.update_rewards:
            self.fitted_reward_optimizer =  torch.optim.Adam(
                self.fitted_reward.parameters(), lr=reward_lr
            )
    ...
    def update_rot(self, obs, L=None, step=None):
        ...
        assert not self.update_rewards, "We do not support updating rot with rewards now."
    ...
    def predict_reward(self, obs, next_obs):
        with torch.no_grad():
            ...
            pred_reward = self.fitted_reward(h, h_next)
            return pred_reward[0]

    def update_inv_and_rewards(self, obs, next_obs, action, L=None, step=None, reward=None):
        assert obs.shape[-1] == 84 and next_obs.shape[-1] == 84

        update_rewards = self.update_rewards and (reward is not None)

        assert update_rewards or self.use_inv, "You need to either update inv or rewards to call this function."
        ...
        if update_rewards:
            ...
            with torch.no_grad():
                reward_max_diff = torch.max(torch.abs(pred_reward - reward))
        ...
        if update_rewards:
            self.fitted_reward_optimizer.zero_grad()

        if update_rewards and self.use_inv:
            (inv_loss + reward_loss).backward()
        elif update_rewards:
        ...
        if update_rewards:
            self.fitted_reward_optimizer.step()
        ...
        if L is not None:
            ...
            if update_rewards:
                L.log('train_reward/reward_loss', reward_loss, step)
                L.log('train_reward/reward_max_diff', reward_max_diff, step)
    ...
    def update(self, replay_buffer, L, step):
        ...
        if self.use_rot and step % self.ss_update_freq == 0:
            self.update_rot(obs, L, step)

        if (self.use_inv or self.update_rewards) and step % self.ss_update_freq == 0:
            self.update_inv_and_rewards(obs, next_obs, action, L, step, reward=reward) # reward is optimized with inv

        if self.use_curl and step % self.ss_update_freq == 0:
            obs_anchor, obs_pos = curl_kwargs["obs_anchor"], curl_kwargs["obs_pos"]
            self.update_curl(obs_anchor, obs_pos, L, step)

    def save(self, model_dir, step, save_optimizers=True, save_target=True):
        torch.save(
        ...
        if self.rot is not None:
            ...
            if save_optimizers:
                torch.save(
                    self.rot_optimizer.state_dict(),
                    '%s/rot_optimizer_%s.pt' % (model_dir, step)
                )
        if self.inv is not None:
            torch.save(
                self.inv.state_dict(),
            ...
            if save_optimizers:
                torch.save(
                    self.inv_optimizer.state_dict(),
                    '%s/inv_optimizer_%s.pt' % (model_dir, step)
                )
        if self.curl is not None:
            torch.save(
            ...
            if save_optimizers:
                torch.save(
                    self.curl_optimizer.state_dict(),
                    '%s/curl_optimizer_%s.pt' % (model_dir, step)
                )
        if self.ss_encoder is not None:
            torch.save(
        ...
        if self.fitted_reward is not None:
            torch.save(
                self.fitted_reward.state_dict(),
                '%s/fit_rewards_%s.pt' % (model_dir, step)
            )
            if self.update_rewards and save_optimizers:
                torch.save(
                    self.fitted_reward_optimizer.state_dict(), '%s/fitted_reward_optimizer_%s.pt' % (model_dir, step)
                )

    def load(self, model_dir, step, load_optimizers=True, load_target=True):
        ...
        if self.rot is not None:
            ...
            if load_optimizers:
                self.rot_optimizer.load_state_dict(
                    torch.load('%s/rot_optimizer_%s.pt' % (model_dir, step))
                )
        if self.inv is not None:
            ...
            if load_optimizers:
                self.inv_optimizer.load_state_dict(
                    torch.load('%s/inv_optimizer_%s.pt' % (model_dir, step))
                )
        if self.curl is not None:
            ...
            if load_optimizers:
                self.curl_optimizer.load_state_dict(
                    torch.load('%s/curl_optimizer_%s.pt' % (model_dir, step))
                )
        if self.ss_encoder is not None:
        ...
        if self.fitted_reward is not None:
            self.fitted_reward.load_state_dict(
                torch.load('%s/fit_rewards_%s.pt' % (model_dir, step))
            )
            if self.update_rewards and load_optimizers:
                self.fitted_reward_optimizer.load_state_dict(
                    torch.load('%s/fitted_reward_optimizer_%s.pt' % (model_dir, step))
                )
